...
public class ServerTest {
	...
	private ConcurrentHashMap<Integer, String> userIDToUsername = new ConcurrentHashMap<Integer, String>(); // id to
																											// username
	private ConcurrentHashMap<String, Integer> usernameToUserID = new ConcurrentHashMap<String, Integer>(); // username
																											// to id
	private ConcurrentHashMap<String, User> allUsers = new ConcurrentHashMap<String, User>(); // map of all existing
																								// users to usernames
		...
		try {
			...
			while (running) {
				...
				case GCL:
					...
					break;
				case CC:
					 handleCreateChatroom(message, output);
					break;
				case IUC:
	...
	private Integer handleLogin(Message message, ObjectOutputStream out) {
		try {
			...
			// User user = new User("admin", "123", true, testUserId++);
	...
	private Integer handleCreateChatroom(Message message, ObjectOutputStream out) {
		try {
			// message variable
			Message Send;
			MessageCreator create;
			...
			Integer lastKey = 0; 
			int creatorID = message.getFromUserID();

			for (Integer key : listOfChatrooms.keySet()) {
			    if (key > lastKey) {
			        lastKey = key; // find the maximum key
			    }
			}
			lastKey++; // increment to make it unique 

			Chatroom chatroom = new Chatroom(lastKey, creatorID);

			listOfChatrooms.put(lastKey, chatroom);
			
			create.setContents("Success");
			create.setChatroom(chatroom);
			create.setToChatroom(lastKey);
			sendMessage(create.createMessage(), out);
	...
	private void handleInviteUserToChatroom() {
	...
	private void handleLeaveChatroom() {
	...
	private void handleSendChatroomMessage(Message message,
			ConcurrentHashMap<Integer, ObjectOutputStream> listOfClients) {
